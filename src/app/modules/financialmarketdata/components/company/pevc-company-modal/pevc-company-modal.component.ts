...
@Component({
  selector: 'app-pevc-company-modal',
  templateUrl: './pevc-company-modal.component.html',
  styleUrls: ['./pevc-company-modal.component.scss'],
})
export class PevcCompanyModalComponent implements OnInit {
  ...
  @Output() selectedPEVCFundCompanyEntityIdFirm = new EventEmitter<any>();
  @Output() selectedPEVCNotListedName = new EventEmitter<any>();
  @Output() selectedPEVCFundName = new EventEmitter<any>();
  totalDataLength: any;
  ...
  handleSelectedRow(data: any) {
    ...
    //     this.selectedPEVCNotListedCompanyEntityId.emit(data.rowData.entityId);
    //     this.selectedPEVCNotListedName.emit(data.rowData.fundFirmName);
    //     this.auth.hidePublicCompany = true;
    //     this.auth.showFundPage = false;
    //   } else if (data.rowData.type === 'fund') {
    //     this.auth.showFundPage = true;
    //     this.selectedPEVCFundCompanyEntityId.emit(data.rowData.entityId);
    //     this.selectedPEVCFundName.emit(data.rowData.fundFirmName);
    //     this.auth.hidePublicCompany = true;
    //     this.auth.showNotListedPage = false;
    //   }
    //   this.auth.showPEVCCompanyModal = false;
    ...
    if (data.rowData.entity_proper_name === data.selEvent.trim()) {
      if (data.rowData.firm_type === '1') {
        ...
        this.auth.showNotListedPage = true;
        this.selectedPEVCNotListedCompanyEntityId.emit(
          data.rowData.factset_pevc_firm_entity_id
        );
        this.selectedPEVCNotListedName.emit(
          data.rowData.entity_proper_name.replaceAll('#', '%23')
        );
        this.auth.hidePublicCompany = true;
        this.auth.showFundPage = false;
      } else if (data.rowData.category === 'PEVC Fund') {
        this.auth.showFundPage = true;
        this.selectedPEVCFundCompanyEntityId.emit(
          data.rowData.factset_fund_entity_id
        );
        this.selectedPEVCFundCompanyEntityIdFirm.emit(
          data.rowData.factset_pevc_firm_entity_id
        );
        this.selectedPEVCFundName.emit(data.rowData.entity_proper_name);
        this.auth.hidePublicCompany = true;
        this.auth.showNotListedPage = false;
      }
      this.auth.showPEVCCompanyModal = false;
